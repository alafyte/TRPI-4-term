    ...
    if (vacancy !== null) {
        return (
            <section className={styles.mainContainer}>
                <div style={{borderBottom: '2px solid #F9F9F9'}}>
                    <div className={styles.header}>
                        <img srcSet={vacancy.logo} alt="logotype"/>
                        <div className={styles.title}>
                            <p className={styles.jobName}>{vacancy.jobName}</p>
                            <p className={styles.companyName}><pre>{vacancy.companyName}   |   {vacancy.city}</pre></p>
                        </div>
                        <a className={styles.close} onClick={() => cancelSelection(vacancy.id)}>
                ...
                <p className={styles.salary}>От {vacancy.description.salaryFrom} до {vacancy.description.salaryTo} руб.</p>
                </div>
                <p className={styles.mainDescription} dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(vacancy.description.description)}}>
                </p>
            </section>
        );
    }
